...
public class TriangularHeightModifierDown implements AmaziaGenerationProbability {
	private final int top, bottom;

	public TriangularHeightModifierDown(final int top, final int bottom) {
		this.top = top;
		this.bottom = bottom;
	}
	...
	public boolean success(final float ability, final BlockPos pos, final ServerWorld world, final Random rand) {
		...
		final int prob = pos.getY() - this.bottom + 1;
		final int r = rand.nextInt(this.bottom + this.top);
		return prob >= r;
	}
